...
function display(name) {
	var txt="<tr><td class='project_name'><div class='btn btn-primary input' onclick='clickonprojects(event)'>"+name+"</div></td><td class='project_button'><div class='btn btn-primary' onclick='del_project(event)' id='input'><i class='fas fa-trash-alt'></i></div></td></tr>";
...
function display_list(i) {
	...
	for(var j=0;j<x.length;j++){
		var msg="<tr><td>"+x[j].list+"</td><td>"+x[j].date+"</td><td>"+x[j].priority+"</td><td><div class='btn btn-primary' onclick='del_lists(event)'><i class='fas fa-trash-alt'></i></div></td></tr>";
		document.getElementById('inf').innerHTML+=msg;
	}
}
